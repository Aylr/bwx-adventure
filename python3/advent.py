...
direction_list  = [ NORTH,  SOUTH,  EAST,  WEST,  UP,  DOWN,  RIGHT,  LEFT,
                    IN,  OUT,  FORWARD,  BACK,
                    NORTHWEST,  NORTHEAST,  SOUTHWEST,  SOUTHEAST] = \
                    list(range(len(direction_names)))
NOT_DIRECTION = None
...
directions = dir_by_name = dict(list(zip(direction_names, direction_list)))
...
class Game(Base):
  ...
  def run_step(self, cmd = None):
    ...
    if words[0].lower() == 'tell' and len(words) > 2:
      (target_name, words) = get_noun(words[1:], list(actor.location.actors.values()))
    ...
    for c in list(actor.location.contents.values()):
        if isinstance(c, Container) and c.is_open:
          things += list(c.contents.values())
      
    potential_verbs = []
    for t in things:
      potential_verbs += list(t.verbs.keys())
    ...
    if target_name:
      for a in list(actor.location.actors.values()):
...
class Location(Lockable):
  ...
  def find_object(self, actor, name):
    ...
    if self.contents:
      ...
      for c in list(self.contents.values()):
        if isinstance(c, Container) and c.is_open() and name in list(c.contents.keys()):
          return c.contents
  ...
  def replace_object(self, actor, old_name, new_obj):
    ...
    if d == None:
      return None
    if not old_name in list(d.keys()):
      return None
    old_obj = d[old_name]
    del d[old_name]
    if new_obj:
...
class Actor(Base):
  ...
  def receive_item(self, giver, thing):
    ...
    if thing in list(self.trades.keys()):
  ...
  def act_give(self, actor, noun, words):
    ...
    if words:
      for w in words:
        if w in list(self.location.actors.keys()):
  ...
  def act_take1(self, actor, noun, words):
    ...
    if not t:
      for c in list(self.location.contents.values()):
        if isinstance(c, Container) and c.is_open:
  ...
  def act_list_verbs(self, actor, noun, words):
    ...
    result = set()
    for t in things:
      for v in list(t.verbs.keys()):
        if len(v.split()) > 1:
          result.add('"' + v + '"')
        else:
          result.add(v);
      for v in list(t.phrases.keys()):
        if len(v.split()) > 1:
          result.add('"' + v + '"')
        else:
          result.add(v);
    self.output((lambda x: x)(" ".join(sorted(result))), FEEDBACK)
    return True
...
class Animal(Actor):
  ...
  def random_move(self, observer_loc):
    ...
    exits = list()
    for (d, c) in list(self.location.exits.items()):
      if c.is_locked():
        continue
